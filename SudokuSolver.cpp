...
void LDS(int** puzzle) {
    ...
    while (!list.empty()) {
        ...
        else { //puzzle is not at the solution depth, so we must expand the tree to the next depth
            Expand(list, n);
        }
...
void Expand(stack<node*>& list, node* parent) {
    ...
    if (row >= 0 && col >= 0) {
        ...
        for (int i = 1; i <= 9; i++) {
            ...
            else { //if the new value is not valid, then we delete this particular child and dont add it to the list
